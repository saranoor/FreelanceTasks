#!/usr/bin/env python3
"""
scrape_fisher_dealers.py
=======================

Purpose
-------
Scrape the FISHER® dealer locator (https://fisherplows.com/dealers/) for ALL dealers
...
- Phone
- Email        (not provided by Fisher dealer pages; left blank)
...
---
    python scrape_fisher_dealers.py \
        ...
        --output fisher_dealers_us_ca.xlsx
...
LOCATOR_URL = "https://fisherplows.com/dealers/"
DEALER_URL_KEYWORD = (
    "/dealer-details/"  # dealer detail pages contain this path fragment
)

DEFAULT_OUTPUT = "fisher_dealers_us_ca.xlsx"
...
def read_postal_codes(csv_path: Path, limit: Optional[int] = None) -> list[str]:
    ...
    if "postal_code" not in df.columns or "country" not in df.columns:
        raise ValueError(
            f"{csv_path} must contain at least columns: country, postal_code"
        )
...
def get_search_input(page):
    # The Fisher locator has a single search input with placeholder "Enter Zip code, city and State".
    selectors = [
    ...
    for sel in selectors:
        ...
        try:
            ...
            continue
    raise RuntimeError(
        "Could not find the ZIP/postal search input on the Fisher locator page."
    )
...
def run_locator_and_collect_dealer_urls(
    ...
    if cache_path.exists():
        try:
            ...
            print(
                f"[resume] Loaded {len(dealer_urls):,} cached dealer URLs from {cache_path}"
            )
        except Exception:
        ...
        for idx, code in enumerate(tqdm(postal_codes, desc="Searching postal codes")):
            ...
            try:
                hrefs = page.eval_on_selector_all(
                    f'a[href*="{DEALER_URL_KEYWORD}"]', "els => els.map(e => e.href)"
                )
                # Filter to Fisher domain and dealer-detail pages
                for h in hrefs:
                    if (
                        isinstance(h, str)
                        and DEALER_URL_KEYWORD in h
                        and h.startswith("https://fisherplows.com/")
                    ):
...
def guess_country_and_state_from_address(address: str) -> tuple[str, str]:
    ...
    if re.search(
        r"\b[ABCEGHJ-NPRSTVXY]\d[ABCEGHJ-NPRSTV-Z][ -]?\d[ABCEGHJ-NPRSTV-Z]\d\b", a
    ):
...
def extract_field_after_label(soup: BeautifulSoup, label_regex: re.Pattern) -> str:
    ...
    if not label_el:
        ...
        for candidate in soup.find_all(["h2", "h3", "h4", "strong", "p", "span"]):
            if candidate.get_text(strip=True) and label_regex.search(
    ...
    if (
        nxt
        and hasattr(nxt, "get_text")
        and label_regex.search(nxt.get_text(strip=True))
    ):
        nxt = nxt.find_next()
    ...
    for _ in range(8):
        ...
        if (
            txt
            and not label_regex.search(txt)
            and txt.lower() not in {"address", "phone"}
        ):
            return txt
        nxt = nxt.find_next()
...
def parse_fisher_dealer_page(
    url: str, session: requests.Session, timeout_s: int = 30
) -> Optional[Dealer]:
    try:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Scrape Fisher dealer locator (US + CA) into Excel."
    )
    ap.add_argument(
        "--postal-codes-file", type=Path, default=Path("postal_codes_us_ca.csv")
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Limit number of postal codes (testing).",
    )
    ap.add_argument(
        "--min-delay",
        type=float,
        default=0.8,
        help="Min delay between searches (seconds).",
    )
    ap.add_argument(
        "--max-delay",
        type=float,
        default=1.8,
        help="Max delay between searches (seconds).",
    )
    ap.add_argument(
        "--threads",
        type=int,
        default=4,
        help="Threads for downloading dealer detail pages.",
    )
    args = ap.parse_args()

    if args.min_delay < 0 or args.max_delay < args.min_delay:
        raise SystemExit(
            "--min-delay must be >= 0 and --max-delay must be >= --min-delay"
        )
    ...
    cache_path = Path("fisher_dealer_urls_cache.json")
    ...
    print(f"dealers URLs sample: {list(dealer_urls)[:5]}")
    if not dealer_urls:
        raise SystemExit(
            "No dealer URLs collected. The site may have blocked requests or the selector logic changed."
        )
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.threads)) as ex:
        futs = {ex.submit(parse_fisher_dealer_page, u, session): u for u in urls}
        for fut in tqdm(
            as_completed(futs), total=len(futs), desc="Scraping dealer pages"
        ):
            d = fut.result()
            if d:
                dealers.append(d)
