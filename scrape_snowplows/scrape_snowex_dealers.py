#!/usr/bin/env python3
"""
scrape_snowex_dealers.py
=======================

Purpose
-------
Scrape the SnowEx® dealer locator (https://snowexproducts.com/dealers/) for ALL dealers
...
- Phone
- Email        (not provided by SnowEx dealer pages; left blank)
...
Run
---
    python scrape_snowex_dealers.py \
        --postal-codes-file postal_codes_us_ca.csv \
        --output snowex_dealers_us_ca.xlsx
...
LOCATOR_URL = "https://snowexproducts.com/dealers/"
DEALER_URL_KEYWORD = "/stores/"  # SnowEx dealer pages live under /stores/

DEFAULT_OUTPUT = "snowex_dealers_us_ca.xlsx"
...
def get_search_input(page):
    # The SnowEx locator has a single search input with placeholder "Enter Zip code, city and State".
    selectors = [
    ...
    for sel in selectors:
        ...
        try:
            ...
            continue
    raise RuntimeError("Could not find the ZIP/postal search input on the SnowEx locator page.")
    ...
    with sync_playwright() as p:
        ...
        for idx, code in enumerate(tqdm(postal_codes, desc="Searching postal codes")):
            ...
            try:
                ...
                # Filter to SnowEx domain and dealer-detail pages
                for h in hrefs:
                    if isinstance(h, str) and DEALER_URL_KEYWORD in h and h.startswith("https://snowexproducts.com/"):
...
def parse_snowex_dealer_page(url: str, session: requests.Session, timeout_s: int = 30) -> Optional[Dealer]:
    try:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Scrape SnowEx dealer locator (US + CA) into Excel.")
    ...
    cache_path = Path("snowex_dealer_urls_cache.json")
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.threads)) as ex:
        futs = {ex.submit(parse_snowex_dealer_page, u, session): u for u in urls}
